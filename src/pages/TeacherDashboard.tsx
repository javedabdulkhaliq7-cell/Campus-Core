...
export default function ParentDashboard({ onLogout }: { onLogout: () => Promise<void> }) {
  ...
  // ── CHANGED: noticesCount → noticesUnread ──
  ...
  // ── CHANGED: loadStats now computes unread notices ──
  async function loadStats(studentId: string) {
    ...
    const noticesQuery = parentSupabase
      .from("announcements")
      .select("id")
      .eq("is_active", true)
      .eq("parent_visible", true);
    if (schoolId) noticesQuery.eq("school_id", schoolId);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl mx-auto p-4 space-y-4">
        ...
        {/* Quick stats — CHANGED: Notices card shows unread count */}
        ...
        {/* Tabs — unchanged structure, notices tab now uses new component */}
          ...
          <div className="flex border-b border-slate-100 overflow-x-auto">
            {TABS.map((tab) => (
              ...
              >
                ...
                {tab.label}
                {/* CHANGED: red dot on Notices tab when unread */}
...
// CHANGED: supports highlight prop for red notices card
